      ...
      when (leftAction?.actionType) {
        AppBarActionType.NAVIGATE_UP -> {
          IconButton(onClick = leftAction.actionFn) {
            Icon(imageVector = Icons.AutoMirrored.Rounded.ArrowBack, contentDescription = stringResource(R.string.navigate_up_description))
          }
        }
        ...
        AppBarActionType.REFRESH_MODELS -> {
          IconButton(onClick = leftAction.actionFn) {
            Icon(
              ...
              contentDescription = stringResource(R.string.refresh_models_description),
